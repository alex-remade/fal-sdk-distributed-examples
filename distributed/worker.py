...
class DistributedWorker:
    ...
    def __call__(self, streaming: bool = False, **kwargs: Any) -> dict[str, Any]:
        """
...
class DistributedRunner:
    ...
    def run(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            async_run = asyncio.run  # type: ignore[assignment]
        ...
        worker.rank_print(f"Running setup for worker with rank {rank}")
        try:
            worker.setup(**kwargs)
        except Exception as e:
            worker.rank_print(f"Error during setup: {e}\n{traceback.format_exc()}")
            socket.send(b"EXIT")
            socket.close()
            return

        setup_duration = time.time() - setup_start
        worker.rank_print(f"Setup took {setup_duration:.2f} seconds")
        ...
        async def execute(payload: bytes) -> Any:
            """
        ...
        async def stream(payload: bytes, as_text_events: bool) -> AsyncIterator[bytes]:
            """
            ...
            with ThreadPoolExecutor() as executor:
                ...
                future = executor.submit(lambda: worker(**payload_dict))
                ...
                while not future.done():
                    try:
                        ...
                        if as_text_events:
                            yield encode_text_event(error_output)
                        else:
                            yield distributed_serialize(error_output)
                        break

                try:
                    result = future.result()
                    if as_text_events:
                        yield encode_text_event(
                            result, is_final=True, image_format=image_format
                        )
                    else:
                        yield distributed_serialize(
                            result, is_final=True, image_format=image_format
                        )
                except Exception as e:
                    error_output = {"error": str(e)}
        ...
        async def exhaust_async_iterator(
            iterator: AsyncIterator[bytes], send_to_socket: bool = False
        ) -> None:
            """
            Exhausts an async iterator to ensure all data is processed.
            :param iterator: The async iterator to exhaust.
            """
            async for datum in iterator:
                if send_to_socket:
                    socket.send(datum)
            if send_to_socket:
                socket.send(b"DONE")
        ...
        if rank == 0:
            ...
            while True:
                ...
                try:
                    if streaming:
                        streamer = stream(serialized_data, as_text_events)
                        async_run(exhaust_async_iterator(streamer, send_to_socket=True))
                    else:
                        result = async_run(execute(serialized_data))
                        socket.send(distributed_serialize(result, is_final=True))
                except Exception as e:
                    error_output = {"error": str(e)}
                    worker.rank_print(
                        f"Error in master worker: {error_output}\n{traceback.format_exc()}"
                    )
                    socket.send(distributed_serialize(error_output, is_final=False))
        else:
            ...
            while True:
                try:
                    ...
                    if payload == b"EXIT":
                        ...
                        break
                    if streaming:
                        streamer = stream(payload, as_text_events)  # type: ignore[arg-type]
                        async_run(exhaust_async_iterator(streamer))
                    else:
                        async_run(execute(payload))  # type: ignore[arg-type]
                except Exception as e:
    ...
    async def stream(
        ...
        await socket.send_multipart(
            [b"0", (b"1" if as_text_events else b"0") + b"1" + payload_serialized]
        )
